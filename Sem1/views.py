...
def index(request):
    ...
    return render(request, 'Sem1/index.html', data)
...
def coin(request):
    ...
    return render(request, 'Sem1/coin.html', context=data)


def rand_num(request):
    num = random.randint(1, 101)
    logger.info(f'Random made - {num}')
    return render(request, 'Sem1/rand_num.html', context={'menu': menu,
...
def get_arts_author(request):
    ...
    return render(request, 'Sem1/get_arts_author.html', context=data)
...
def get_article(request, art_id: int):
    ...
    article.view += 1
    # article.save()
    return render(request, 'Sem1/get_article.html',
...
def choose_game(request):
    if request.method == 'POST':
        ...
        form = Choose_game()
    return render(request, 'Sem1/choose_game.html', {'form': form})
...
def add_author(request):
    ...
    return render(request, 'Sem1/new_author.html', {'form': form, 'message': message})
...
def add_article(request):
    ...
    return render(request, 'Sem1/new_article.html', {'form': form, 'message': message})
...
def update_production(request, product_pk):
    ...
    return render(request, 'Sem1/update_prod.html', {'form': form})

# def update_production(request, product_id):
#     product = get_object_or_404(Production, pk=product_id)  # Получаем объект товара, который нужно отредактировать
#     if request.method == 'POST':  # Если форма отправлена
#         form = UpdateProduction(request.POST, instance=product)  # Создаем экземпляр формы, связанный с объектом товара
#         if form.is_valid():  # Проверяем, прошла ли валидация формы
#             form.save()  # Сохраняем изменения
#             # Перенаправляем на страницу с деталями продукта после сохранения
#     else:
#         form = UpdateProduction(instance=product)  # Создаем экземпляр формы, заполненный текущими данными товара
#     return render(request, 'Sem1/update_production.html', {'form': form})  # Отображаем форму редактирования товара

# def update_production(request, prod_pk):
#     product = get_object_or_404(Production, pk=prod_pk)
#     if request.method == 'POST':
#         form = UpdateProduction(request.POST)
#         message = "Данные некоректны"
#         if form.is_valid():
#             new_name = form.cleaned_data['name']
#             new_description = form.cleaned_data['description']
#             new_price = form.cleaned_data['price']
#             new_quontity = form.cleaned_data['quontity']
#             product.name = new_name
#             product.description = new_description
#             product.price = new_price
#             product.quontity = new_quontity
#             product.save()
#             message = 'Изменения сохранены'
#             return redirect(update_production, prod_pk)
#     else:
#         form = UpdateProduction(request)
#         message = "Измените требуемый параметр"
#     return render(request, 'Sem1/update_prod.html', {'form': product, 'message': message})
